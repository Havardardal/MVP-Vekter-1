...
export default function CriticalitySelector({ value, onChange }) {
  ...
  const info = infoLevel ? CRITICALITY_INFO[infoLevel] : null;
  ...
  return (
    <div className="criticality-wrapper">
      <div className="criticality-selector">
        ...
        {[1, 2, 3, 4, 5].map((level) => (
          <button
            key={level}
            type="button"
            className={`criticality-btn level-${level}${value === level ? " selected" : ""}`}
      ...
      {info && (
        <div className={`criticality-info level-${infoLevel}-info`}>
